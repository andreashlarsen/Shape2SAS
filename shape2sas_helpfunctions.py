...
def calc_all_dist_func(point_distribution) -> np.ndarray:
    """
    Calculate unique pairwise distances between 3D points.
    Returns a 1D float32 array of length N*(N-1)/2.
    """
    #x_in,y_in,z_in = point_distribution.x,point_distribution.y,point_distribution.z
...
def calc_all_contrasts_func(point_distribution) -> np.ndarray:
    """
...
def generate_histogram_func(dist: np.ndarray, prpoints: int, contrast: np.ndarray, r_max: float) -> Vector2D:
    """
...
def calc_hr_func(dist: np.ndarray, prpoints: int, contrast: np.ndarray, polydispersity: float) -> Vector2D:
    """
    calculate h(r)
...
def calc_Rg_func(r: np.ndarray, pr: np.ndarray) -> float:
    """ 
...
def calc_Pq_func(q: np.ndarray, r: np.ndarray, pr: np.ndarray, conc: float, volume_total: float) -> Vector2D:
    """
...
def calc_Iq_func(q: np.array, Pq: np.ndarray, S_eff: np.ndarray, sigma_r: float) -> np.ndarray:
    """
...
def calc_com_dist_func(point_distribution) -> np.ndarray:
    """ 
...
def calc_A00_func(q,point_distribution) -> np.ndarray:
    """
...
def decoupling_approx_func(q,point_distribution, Pq: np.ndarray, S: np.ndarray) -> np.ndarray:
    """
...
#     @staticmethod
#     def generate_histogram(dist: np.ndarray, prpoints: int, contrast: np.ndarray, r_max: float) -> Vector2D:
#         """
#         make histogram of point pairs, h(r), binned after pair-distances, r
#         used for calculating scattering (fast Debye)
...
#         input
#         dist     : all pairwise distances
#         prpoints : number of points in h(r)
#         contrast : contrast of points
#         r_max    : max distance to include in histogram
...
#                 dmax = np.amax(dist) * (1 + 3 * polydispersity)
#             r_max = dmax * ratio_rmax_dmax
#             r, hr_1 = self.generate_histogram(dist, prpoints, contrast, r_max)
#             N_poly_integral = 25 # should be uneven to include 1 in factor_range (precalculated)
#             hr  = np.zeros_like(hr_1, dtype=np.float32)
#             #norm = 0.0
#             if lognormal:
...
#             dmax = np.amax(dist)
#             r_max = dmax * ratio_rmax_dmax
#             r, hr = self.generate_histogram(dist, prpoints, contrast, r_max)
...
#     def calc_pr(self, prpoints: int, polydispersity: float) -> Vector3D:
#         """
#         calculate p(r)
#         p(r) is the contrast-weighted histogram of distances, without the self-terms (dist = 0)
...
#         #r, pr = self.calc_hr(dist[idx_nonzero], prpoints, contrast[idx_nonzero], polydispersity)
#         printt('        calculating pr...')
#         r, pr = self.calc_hr(dist, prpoints, contrast, polydispersity)
