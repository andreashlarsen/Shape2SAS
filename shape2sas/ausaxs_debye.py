...
def distance_histogram(x: np.ndarray, y: np.ndarray, z: np.ndarray, w: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    raises ImportError if pyausaxs is not installed, or RuntimeError if the AUSAXS backend fails to evaluate the calculation.
    """
    import pyausaxs as ausaxs
    _configure()
    ...
    # AUSAXS's histogram sums the contrast product over every ordered pair (i, j), including self-terms (i == j) landing in the r=0 bin.
    # Shape2SAS only wants the sum over unordered pairs (i, j) with i != j, so we need to subtract the self-terms and divide by 2.
